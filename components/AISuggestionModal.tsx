...
const AISuggestionModal: React.FC<AISuggestionModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[300] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-500">
            <div className="w-full max-w-2xl bg-zinc-900 border border-emerald-500/30 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                {/* Header */}
                <div className="bg-gradient-to-r from-emerald-900/40 via-zinc-900 to-cyan-900/40 p-8 border-b border-emerald-500/10 relative">
                    <div className="flex items-center gap-4 mb-2">
                        <div className="p-3 bg-emerald-500/20 rounded-2xl animate-pulse">
                            <BrainCircuit className="w-8 h-8 text-emerald-400" />
                        </div>
                        <div>
                            <h2 className="text-3xl font-black text-white tracking-tight">AI 智能處方建議系統</h2>
                            <div className="flex items-center gap-2 mt-1">
                                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" />
                                <span className="text-emerald-500 text-[10px] font-bold uppercase tracking-widest">Data-Driven Clinical Reasoning Active</span>
                            </div>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="absolute right-6 top-6 w-10 h-10 bg-white/5 hover:bg-white/10 rounded-full flex items-center justify-center text-zinc-500 transition-all border border-white/5"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                    ...
                    <section className="space-y-4">
                        <div className="flex items-center gap-2 text-zinc-400">
                            ...
                            <h3 className="text-sm font-bold uppercase tracking-widest">1. 歷史數據檢索與臨床分析 (Analysis)</h3>
                        </div>
                        <div className="bg-zinc-950/50 rounded-2xl p-6 border border-zinc-800 leading-relaxed">
                            <p className="text-zinc-300 text-sm italic">{analysis}</p>
                        </div>
                    </section>

                    {/* Section 2: Clinical Advice */}
                    <section className="space-y-4">
                        <div className="flex items-center gap-2 text-zinc-400">
                            <Heart className="w-4 h-4 text-rose-400" />
                            <h3 className="text-sm font-bold uppercase tracking-widest">2. 智能臨床建議 (Clinical Advice)</h3>
                        </div>
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-zinc-950/50 rounded-xl p-4 border border-zinc-800">
                                <span className="text-[10px] font-bold text-zinc-500 uppercase block mb-1">建議遊戲名稱</span>
                                <span className="text-white font-bold">{recommended_config.game_topic}</span>
                            </div>
                            <div className="bg-zinc-950/50 rounded-xl p-4 border border-zinc-800">
                                <span className="text-[10px] font-bold text-zinc-500 uppercase block mb-1">訓練模式</span>
